...
Para $k=0,1,\ldots,N-1$. Con la biblioteca de complejos, esto se puede traslador directamente a código de una forma sencilla:
...
Podemos observar que el resultado de este ejemplo es prácticamente 0.0, salvo para los valores con $k=8,92$, ya que $92=100-8\equiv -8$ (la transformada de Fourier es simétrica respecto de $k=0$ y tiene picos en las frecuencias de la función senoidal). 
...
- Distribución gaussiana de media $\mu$ y varianza $\sigma ^2$:  `normal_distribution<double>(mu, sigma)`.
